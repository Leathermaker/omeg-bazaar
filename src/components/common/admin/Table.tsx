...
const Table = <T extends { [key: string]: unknown }>({ columns, data }: TableProps<T>) => {
  ...
  return (
    <div ref={tableRef} className="overflow-hidden p-4 relative">
      <table className="w-full text-left mt-4">
        <thead>
          <tr className="bg-[#F4F4F4] text-[#000000] font-poppins font-semibold text-sm leading-normal">
            {columns.map((column, index) => (
              <th
                key={index}
                className={`py-3 px-4 text-center ${column.bg ? column.bg : ""} 
        ...
        <tbody className="text-[#333333] font-poppins text-sm">
          {data.map((row, rowIndex) => (
            <tr key={rowIndex} className="relative">
              {columns.map((column, colIndex) => (
                <td
                  key={colIndex}
                  className={`py-4 px-4 text-center ${column.bg ? column.bg : ""} 
                  ${column.key !== "action" ? "border-r-[3px] border-r-[#F1F1F1]" : ""} 
                  ${column.key !== "checkbox" ? "border-b border-b-[#000000]" : ""}`}
                >
                  {column.key === "checkbox" ? (
                    <input type="checkbox"  />
                  ) : column.key === "action" ? (
                    <button
                      className="p-2 text-[#000000] font-extrabold text-2xl "
              ...
              {/* Show DialogBox for the clicked row */}
              {openDialog === rowIndex && (
                <div
                  className={`absolute right-0 z-10 ${dialogPosition === "top" ? "bottom-full mb-2" : "top-full mt-2"}`}
                >
                  <DialogBox />
                </div>
              )}
